...
The performance was designed to be in quadrophonic, and the positions of the sounds were controlled independently of each other, using the Max for Live plugin the Surround Panner:
...
I wanted this sound to move back and forth around the room, with the speed in the movement being changed by the amount of color change.   This would cause the sound to move quickly back and forth when the visuals were intense.  I originally used LFOs in max for live which controlled x and y, but this ended up taking a lot of CPU when more midi values were sent.  I changed this to be calculated in TouchDesigner, which is a much more performance application.  These calculations in TouchDesigner were sent as midi control values and that would be mapped to x and y of the sound in Ableton.

{{< original_image src="images/surroundpanner.gif" >}}

##### What went wrong during the performance.

Right before the performance I tested this out by holding up water paint containers in front of the camera to see how it would affect the positional change.  As it wasn't affecting it much, I turned up the sensitivity exponentially, without properly testing this out.  When I performed on stage and used actualy drops of liquid the detection was much stronger, calling the exponentially increased value to be too big and making the sound jitter around super stronly, creating a glitchy affect.  I rescued this by going to the computer and disabling the surround panner for this sound, making it be mono through the two front speakers.  No one seemed to notice after even though I internally freaked out.
...
As closure for this semester, I hooked up the system to the Buchla 200e at Steinhardt and got some basic patches working.  I Everything sounded so good on the real thing!  More to be said about that integration, but here are some videos: