            ...
            <nav className="mt-6">
              <ul className="-my-2 divide-y divide-zinc-100 text-base text-zinc-800 dark:divide-zinc-100/5 dark:text-zinc-300">
                {navItems.map((navItem) => (
                  <MobileNavItem key={navItem.path} href={navItem.path}>
                    {navItem.text}
                  </MobileNavItem>
                ))}
              </ul>
  ...
  return (
    <li>
      <Link
        href={href}
        className={clsx(
          'relative block px-3 py-2 transition font-mono',
  ...
  return (
    <nav {...props}>
      <ul className="flex rounded-full bg-white/90 px-3 text-sm font-medium text-zinc-800 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10">
        {navItems.map((navItem) => (
          <NavItem key={navItem.path} href={navItem.path}>
            {navItem.text}
          </NavItem>
        ))}
      </ul>
...
function Avatar({ large = false, className, ...props }: any) {
  return (
    <Link href="/" aria-label="Home" title="Home" className={clsx(className, 'pointer-events-auto')} {...props}>
      <Image
        src={avatarImage}
        alt=""
...
export function Header() {
  const isHomePage = false; //useRouter().pathname === '/';
